...
export class CuadreCajaService {
    ...
    async findAll(): Promise<CuadreCaja[]> {
    ...
    async update(id: string, updateCuadreCajaDto: UpdateCuadreCajaDto): Promise<CuadreCaja> {
        if (updateCuadreCajaDto.empleado) {
            const existe = await this.usuarioModel.findById(updateCuadreCajaDto.empleado);
            if (!existe) throw new NotFoundException('El empleado no existe');
        }

        const updateCuadre = await this.cuadreCajaModel
            .findByIdAndUpdate(id, updateCuadreCajaDto, { new: true })
            .exec();
    ...
    async remove(id: string): Promise<void> {
        const deleteCuadre = await this.cuadreCajaModel.findByIdAndDelete(id).exec();
        if (!deleteCuadre) {
            throw new NotFoundException('No se encontró el cuadre de caja');
        }
    }
