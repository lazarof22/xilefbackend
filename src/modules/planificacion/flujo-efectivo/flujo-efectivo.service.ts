...
export class FlujoEfectivoService {
  ...
  async findByPeriodo(periodo: string): Promise<ProyeccionFlujo[]> {
    return this.proyeccionModel.find({ periodo }).sort({ fecha: 1 }).exec();
  }

  async proyectarFlujoDiario(
    periodo: string,
  ): Promise<{ proyecciones: ProyeccionFlujo[]; saldoAcumulado: number }> {
    const proyecciones = await this.proyeccionModel
      .find({ periodo })
      .sort({ fecha: 1 })
      .exec();
    ...
    for (const p of proyecciones) {
      saldoAcumulado = Number(
        (saldoAcumulado + p.flujoNetoProyectado).toFixed(2),
      );
    }

    return { proyecciones, saldoAcumulado };
  }

  async generarAlerta(
    periodo: string,
    umbralMinimo: number,
  ): Promise<{ alerta: boolean; proyeccionesCriticas: ProyeccionFlujo[] }> {
    const proyecciones = await this.proyeccionModel
      .find({ periodo })
      .sort({ fecha: 1 })
      .exec();
    ...
    let saldoAcumulado = 0;
    const criticas: ProyeccionFlujo[] = [];

    for (const p of proyecciones) {
      saldoAcumulado = Number(
        (saldoAcumulado + p.flujoNetoProyectado).toFixed(2),
      );
      if (saldoAcumulado < umbralMinimo) {
        criticas.push(p);
      }
    }

    return {
      alerta: criticas.length > 0,
      proyeccionesCriticas: criticas,
    };
  }
}
