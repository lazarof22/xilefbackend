...
export class PlanificacionCobrosController {
  ...
  @ApiOperation({ summary: 'Obtener planes de cobro vencidos' })
  @ApiResponse({ status: 200, description: 'Planes de cobro vencidos' })
  findVencidos() {
    return this.planificacionService.findVencidos();
  }

  @Get('consulta/cliente')
  @ApiOperation({ summary: 'Obtener planes de cobro por cliente' })
  @ApiQuery({
    name: 'clienteId',
    required: true,
    description: 'ID del cliente',
  })
  @ApiResponse({ status: 200, description: 'Planes de cobro del cliente' })
  findByCliente(@Query('clienteId') clienteId: string) {
    return this.planificacionService.findByCliente(clienteId);
  }

  @Get('consulta/morosidad')
  @ApiOperation({ summary: 'Proyectar morosidad con X días de atraso' })
  @ApiQuery({ name: 'diasMora', required: true, description: 'Días de mora' })
  @ApiResponse({ status: 200, description: 'Cobros con morosidad' })
  proyectarMorosidad(@Query('diasMora') diasMora: string) {
    return this.planificacionService.proyectarMorosidad(Number(diasMora));
  }
