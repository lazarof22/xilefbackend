...
export class BancoService {
  ...
  async create(createDto: CreateBancoDto): Promise<Banco> {
    const existente = await this.bancoModel
      .findOne({ codigoBanco: createDto.codigoBanco })
      .exec();
    if (existente)
      throw new BadRequestException('Ya existe una cuenta con ese código');
    const saldoInicial = createDto.saldoInicial ?? 0;
    const created = new this.bancoModel({
      ...createDto,
      saldoInicial,
  ...
  async update(id: string, updateDto: UpdateBancoDto): Promise<Banco> {
    if (!isValidObjectId(id))
      throw new NotFoundException('Cuenta bancaria no encontrada');
    const updated = await this.bancoModel
  ...
  async getSaldos(): Promise<SaldoResponse[]> {
    const cuentas = await this.bancoModel
      .find({ activo: true })
      .populate('moneda')
      .exec();
