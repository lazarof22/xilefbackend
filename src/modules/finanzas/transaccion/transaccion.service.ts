...
import { TipoTransaccion, ResumenTransacciones } from './types/transaccion.types';

@Injectable()
export class TransaccionService {
  constructor(@InjectModel(Transaccion.name) private transaccionModel: Model<TransaccionDocument>) {}

  async create(createDto: CreateTransaccionDto): Promise<Transaccion> {
    const existente = await this.transaccionModel.findOne({ codigo: createDto.codigo }).exec();
    if (existente) throw new BadRequestException('Ya existe una transacción con ese código');
  ...
  async findAll(): Promise<Transaccion[]> {
    return this.transaccionModel.find()
      .populate('categoria').populate('moneda').populate('cuentaBancaria')
      .populate('cliente').populate('proveedor')
      .sort({ fecha: -1 }).exec();
  }

  async findOne(id: string): Promise<Transaccion> {
    if (!isValidObjectId(id)) throw new NotFoundException('Transacción no encontrada');
    const doc = await this.transaccionModel.findById(id)
      .populate('categoria').populate('moneda').populate('cuentaBancaria')
      .populate('cliente').populate('proveedor').exec();
    if (!doc) throw new NotFoundException('Transacción no encontrada');
    return doc;
  }

  async update(id: string, updateDto: UpdateTransaccionDto): Promise<Transaccion> {
    if (!isValidObjectId(id)) throw new NotFoundException('Transacción no encontrada');
    const updated = await this.transaccionModel.findByIdAndUpdate(id, updateDto, { new: true }).exec();
  ...
  async getPorPeriodo(desde: string, hasta: string): Promise<Transaccion[]> {
    ...
    }).populate('categoria').populate('moneda').populate('cuentaBancaria').sort({ fecha: -1 }).exec();
  }

  async getResumen(desde?: string, hasta?: string): Promise<ResumenTransacciones> {
    const filtro: any = {};
    if (desde && hasta) filtro.fecha = { $gte: new Date(desde), $lte: new Date(hasta) };

    const ingresos = await this.transaccionModel.aggregate([
      { $match: { ...filtro, tipo: TipoTransaccion.INGRESO } },
      { $group: { _id: null, total: { $sum: '$monto' }, cantidad: { $sum: 1 } } },
    ]).exec();
    const egresos = await this.transaccionModel.aggregate([
