...
import {
  TarjetaCombustible,
  TarjetaCombustibleDocument,
} from './schema/tarjeta-combustible.schema';
import {
  CargaCombustible,
  CargaCombustibleDocument,
...
export class CombustibleService {
  constructor(
    ...
    @InjectModel(TarjetaCombustible.name)
    private tarjetaModel: Model<TarjetaCombustibleDocument>,
    @InjectModel(CargaCombustible.name)
    private cargaModel: Model<CargaCombustibleDocument>,
  ) {}

  async createVehiculo(createDto: CreateVehiculoDto): Promise<Vehiculo> {
    const existente = await this.vehiculoModel
      .findOne({ codigo: createDto.codigo })
      .exec();
    if (existente)
      throw new BadRequestException('Ya existe un vehículo con ese código');
  ...
  async createTarjeta(
    createDto: CreateTarjetaDto,
  ): Promise<TarjetaCombustible> {
    const existente = await this.tarjetaModel
      .findOne({ numeroTarjeta: createDto.numeroTarjeta })
      .exec();
    if (existente)
  ...
  async createCarga(createDto: CreateCargaDto): Promise<CargaCombustible> {
    const existente = await this.cargaModel
      .findOne({ codigo: createDto.codigo })
      .exec();
    if (existente)
      throw new BadRequestException('Ya existe una carga con ese código');
    const created = new this.cargaModel(createDto);
    return created.save();
  }

  async findAllCargas(): Promise<CargaCombustible[]> {
    return this.cargaModel
      .find()
      .populate('tarjeta')
      .populate('vehiculo')
      .sort({ fecha: -1 })
