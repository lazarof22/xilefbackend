...
export class CuentaPagarService {
  constructor(@InjectModel(CuentaPagar.name) private cxpModel: Model<CuentaPagarDocument>) {}

  async create(createDto: CreateCuentaPagarDto): Promise<CuentaPagar> {
    const existente = await this.cxpModel.findOne({ codigo: createDto.codigo }).exec();
    if (existente) throw new BadRequestException('Ya existe una CxP con ese código');
    const created = new this.cxpModel({ ...createDto, saldoPendiente: createDto.montoOriginal });
    return created.save();
  }

  async findAll(): Promise<CuentaPagar[]> {
    return this.cxpModel.find().populate('proveedor').populate('concepto').sort({ fechaEmision: -1 }).exec();
  }

  async findOne(id: string): Promise<CuentaPagar> {
    if (!isValidObjectId(id)) throw new NotFoundException('Cuenta por pagar no encontrada');
    const doc = await this.cxpModel.findById(id).populate('proveedor').populate('concepto').exec();
    if (!doc) throw new NotFoundException('Cuenta por pagar no encontrada');
    return doc;
  }

  async update(id: string, updateDto: UpdateCuentaPagarDto): Promise<CuentaPagar> {
    if (!isValidObjectId(id)) throw new NotFoundException('Cuenta por pagar no encontrada');
    const updated = await this.cxpModel.findByIdAndUpdate(id, updateDto, { new: true }).exec();
  ...
  async abonar(id: string, abono: { monto: number; fechaPago?: string; referencia?: string }): Promise<CuentaPagar> {
    if (!isValidObjectId(id)) throw new NotFoundException('Cuenta por pagar no encontrada');
    const cxp = await this.cxpModel.findById(id).exec();
    if (!cxp) throw new NotFoundException('Cuenta por pagar no encontrada');
    if (cxp.estado === EstadoCxP.PAGADA) throw new BadRequestException('La cuenta ya está pagada');
    if (abono.monto <= 0) throw new BadRequestException('El monto debe ser mayor a 0');
    if (abono.monto > cxp.saldoPendiente) throw new BadRequestException('El monto excede el saldo pendiente');
  ...
  async getResumen(): Promise<any> {
    const totalPendiente = await this.cxpModel.aggregate([
      { $match: { estado: { $ne: EstadoCxP.PAGADA } } },
      { $group: { _id: null, total: { $sum: '$saldoPendiente' } } },
    ]).exec();
    const porEstado = await this.cxpModel.aggregate([
      { $group: { _id: '$estado', cantidad: { $sum: 1 }, total: { $sum: '$saldoPendiente' } } },
    ]).exec();
    return { totalPendiente: totalPendiente.length > 0 ? totalPendiente[0].total : 0, porEstado };
  }
}
